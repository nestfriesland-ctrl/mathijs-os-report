...
(function () {
  ...
  // --- Raw OHLC for charts -------------------------------------------
  // Returns an array of { time, open, high, low, close } in ascending time
  // order, matching lightweight-charts' candlestick/line series shape.
  // `pair` is the Kraken pair string (e.g. 'XBTUSD', 'XETHXXBT'),
  // `intervalMin` is the Kraken interval (1, 15, 60, 240, 1440), and `count`
  // caps how many *closed* candles to return (the still-forming candle is
  // dropped). 5-minute cache keyed on (pair, interval, count).
  const candleCache = {};
  const CANDLE_CACHE_TTL_MS = 5 * 60 * 1000;

  async function fetchCandles(pair, intervalMin, count) {
    const key = `${pair}:${intervalMin}:${count}`;
    const cached = candleCache[key];
    if (cached && (Date.now() - cached.ts) < CANDLE_CACHE_TTL_MS) {
      return cached.data;
    }
    try {
      const url = `${BASE}/OHLC?pair=${pair}&interval=${intervalMin}`;
      const r = await fetch(url, { cache: 'no-store' });
      if (!r.ok) return cached ? cached.data : null;
      const data = await r.json();
      // Kraken nests under .result with the pair-key; try the first non-`last`
      // entry (handles both XBTUSD->XXBTZUSD aliasing and bare keys).
      let arr = null;
      if (data.result) {
        for (const k of Object.keys(data.result)) {
          if (k === 'last') continue;
          if (Array.isArray(data.result[k])) { arr = data.result[k]; break; }
        }
      }
      if (!arr || !arr.length) return cached ? cached.data : null;
      // Drop the still-forming last candle so the chart doesn't redraw it
      // every refresh; take last `count` closed candles.
      const closed = arr.slice(0, -1).slice(-count);
      const candles = closed.map(row => ({
      ...
      candleCache[key] = { ts: Date.now(), data: candles };
      return candles;
    } catch (e) {
  ...
  window.Kraken = {
    ...
    getAsset, fetchTickers, fetchOHLC, fetchAllAnchors, fetchCandles,
  };
})();
